...
describe('audioGenerator', () => {
    ...
    it('should convert text to speech and save as MP3', async () => {
        ...
        jest.spyOn(textToSpeech, 'TextToSpeechClient').mockImplementation(() => ({
            synthesizeSpeech: jest.fn().mockResolvedValue([{ audioContent: 'fake_audio_content' }]),
        }));
        ...
        expect(fs.outputFile).toHaveBeenCalledWith(outputPath, 'fake_audio_content', 'binary');
    });

    it('should handle errors during speech synthesis', async () => {
        // Mocking an error scenario
        jest.spyOn(textToSpeech, 'TextToSpeechClient').mockImplementation(() => ({
            synthesizeSpeech: jest.fn().mockRejectedValue(new Error('synthesis error')),
        }));

        const line = { content: 'Hello, world!' };
        const outputPath = 'output.mp3';

        await expect(convertTextToSpeech(line, outputPath)).rejects.toThrow('synthesis error');
    });
});
